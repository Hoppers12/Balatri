...
public final class GameController {
	...
	/**
	 * Builds the controller wiring the game state, a view and the session high score.
	 *
	 * @param state     the game state to drive
	 * @param view      the view used for display and interaction
	 * @param highScore the session high score to update at the end
	...
	/**
	 * Runs the main loop until the game is over, then submits the result to the high score
	...
	public void run() {
		...
		var record = highScore.submit(state.getTotalScore(), state.getBlindsBeaten());
		view.showEnd(state, highScore, record);
	}

	/**
	 * Plays one round: draw eight cards, then let the player discard (and redraw) or play
	 * a five-card hand. On a played hand, score it, apply blind progression and award a
	 * planet when the blind is beaten.
	 */
	private void playOneRound() {
		view.showState(state);
		// Pioche les cartes du tour (8 par défaut, cf. Hand.CARDS_DRAWN)
		var cards = new ArrayList<>(View.sortByRank(state.getDeck().draw(GameState.CARDS_DRAWN)));
		...
		while (!hasPlayed) {
			...
			if (selectedObject.isDiscardAction()) {
				...
				// Blind battu si le blind a changé
	...
	/**
	 * Picks a random planet.
	 *
	 * @return a randomly chosen planet
	 */
